...
namespace The_Party_Animals
{
    public class Dragable : Component
    {
        ...
        int CheckCollision()
        {
            if (_rectangle.Y > Game1.ObjectSpace.Bottom)
            {
                _collided = true;
                currentDirecton.Y *= -1;
                return 3;
            }
            else if (_rectangle.X > Game1.ObjectSpace.Right)
            {
                _collided = true;
                currentDirecton.X *= -1;
                return 2;
            }
            else if (_rectangle.X < Game1.ObjectSpace.Left)
            {
                _collided = true;
                currentDirecton.X *= -1;
                return 1;
            }
            else if (_rectangle.Y < Game1.ObjectSpace.Top)
            {
                _collided = true;
                currentDirecton.Y *= -1;
                return 0;
